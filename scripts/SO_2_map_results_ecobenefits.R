...
e <- try(setwd(paste0(wd,'/OUTPUT/Treatment_plan_ecobenefits'))) #===> Change directory
...
#-> Maps
setwd(paste0(wd,'/OUTPUT/Treatment_plan_ecobenefits/Maps')) #===> Change directory
for(i in 1:nrow(tspecs)){
...
vmax <- vector()
for(i in 1:nrow(tspecs)){
	EB[[i]] <- raster(paste0(tspecs$FB_Code[i],'_EB.tif'))*ra_extent
	EB[[i]] <- EB[[i]]*Fs[[i]] # Limit to feasible
	vmax[i] <- quantile(EB[[i]][EB[[1]] > 0],0.975,na.rm=T) # Grabbing the 97.5th per. of pos. vals.
}

#-> Define bins and colors
bins <- seq(-max(vmax),max(vmax),max(vmax)/4.5) # 9 equal interval bins
llabels <- format(round(bins[-1],0),scientific=F,big.L=3,big.mark=',',trim=T,drop0trailing=T)
l2 <- rep('',length(llabels)) # Add high and low labels for clarity
l2[1] <- ' - low'
l2[length(l2)] <- ' - high'
llabels <-paste0(llabels,l2)
cols <- colorRampPalette(c('red','yellow','forestgreen'))(length(bins)-1)

#-> Maps
setwd(paste0(wd,'/OUTPUT/Treatment_plan_ecobenefits/Maps')) #===> Change directory
for(i in 1:nrow(tspecs)){
	tiff(paste0(tspecs$FB_Code[i],'_ecological_benefit.tif'),width=ms$Width_pixels,
	     height=ms$Height_pixels,pointsize=ms$Pointsize,compression='lzw',type='windows')
	par(mar=c(0.6,0.6,2.5,0.6))
	plot(hillshade,col=SoG,axes=F,box=F,legend=F,asp=1,maxpixels=5000000)
	EB[[i]][EB[[i]]>max(vmax)] <- max(vmax[i]) # Drop top 2.5% for display purposes
	plot(EB[[i]],breaks=bins,col=cols,alpha=0.6,axes=F,legend=F,box=F,maxpixels=5000000,add=T)
	title(main=paste0(tspecs$Treatment[i],' Ecological Benefit'),cex.main=1.75)
	plot(rivers_fc,col='blue',lwd=1.5,add=T)
	plot(roads_fc,col='grey20',lwd=1.5,add=T)
	plot(ra_extent_fc,border='black',lwd=2,add=T)
	legend(as.character(ms$Legend_pos),llabels,fill=cols,title=expression(bold('Eco. Benefit')),
		   bty='n')
	plotRef(bb=ra_extent_fc,pos=as.character(ms$ScaleArrow_pos),blength=ms$ScaleBar_mi)
	g <- dev.off()
	trimLegend(paste0(tspecs$FB_Code[i],'_ecological_benefit.tif'),ms$Width_pixels,ms$Height_pixels,
	           0.925)
}
...
#-> Define bins and colors
bins <- c(0,1000,2000,3000,4000,5000,6000,7000,8000,9000,10000,round(max(vmax),-3))   # INCREASED BINS FOR MAPPING CLARITY #
...
for(i in 1:nrow(tspecs)){
	CE[[i]] <- EB[[i]]/Cs[[i]]
	vmax[i] <- cellStats(CE[[i]],'max') # Straight maximum for defining bins
	#vmax[i] <- quantile(values(CE[[i]]),0.975,na.rm=T)
	
	############################## STEPHANIE ADD ################################
	# output as rasters
	writeRaster(suppressWarnings(CE[[i]]*ra_extent),paste0('GIS_', tspecs$FB_Code[i],'_cost_effectiveness.tif'),
	            format='GTiff',overwrite=T)
}

#-> Define bins and colors
bins <- seq(-max(vmax),max(vmax),max(vmax)/4.5) # 9 equal interval bins
llabels <- format(round(bins[-1],2),scientific=F,big.L=3,big.mark=',',drop0trailing=T)
l2 <- rep('',length(llabels)) # Add high and low labels for clarity
l2[1] <- ' - low'
l2[length(l2)] <- ' - high'
llabels <-paste0(llabels,l2)
...
title(main='Treatment Priorities',cex.main=1.75)
plot(rivers_fc,col='blue',lwd=1.5,add=T)
plot(roads_fc,col='grey20',lwd=1.5,add=T)
plot(ra_extent_fc,border='black',lwd=2,add=T)
