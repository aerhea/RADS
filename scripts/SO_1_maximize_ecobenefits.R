####################################################################################################
#### Spatially optimize treatments to maximize ecological benefits
...
###########################################START MESSAGE############################################
log <- file(paste0(wd,'/OUTPUT/Treatment_plan_ecobenefits/so_treatments.log'))
...
###---> Empty output folder
e <- try(setwd(paste0(wd,'/OUTPUT/Treatment_plan_ecobenefits'))) #===> CD
...
tua[,2] <- tua[,2]*(prod(res(rTUs))/10000)*2.47105 # Convert to acres
...
###---> Ecological benefits
# Note that you don't need to do the pixel to acre conversion since the ecological benefits
# are relative.
setwd(paste0(wd,'/OUTPUT/EBA_dcNVC/GIS')) #===> Change directory
EBs <- list() # Treatment benefit rasters
for(i in 1:nrow(tspecs)){
	#-> Process feasibility raster for filtering
	nfeas <- Fs[[i]] # Create null version of feasibility raster
	nfeas[nfeas==0] <- NA
	#-> load in treatment benefit effect rasters
	eb <- raster(paste0(tspecs$FB_Code[i],'_dcNVC.tif'))
	crs(eb) <- crs(rTUs)
	eb <- eb*nfeas*ma_extent
	EBs[[i]] <- eb
}

#-> Calculate cost for each treatment
for(i in 1:nrow(tspecs)){
	zt <- zonal(EBs[[i]],rTUs,'mean',na.rm=T)
	tua$X <- zt[,'mean']
	tua$X[is.na(tua$X)] <- 0 # Set NAs to zero
	colnames(tua)[ncol(tua)] <- paste0('T',i,'_EB')
}
...
dv.list <- list()
for(i in 1:nrow(tspecs)){
	dv <- tua[,c('UID','Tot_feas',paste0('T',i,'_feas'),paste0('T',i,'_EB'),
	             paste0('T',i,'_Cost'))]
	dv$TrtType <- i
	colnames(dv) <- c('UID','TotFeasAcre','FeasAcre','BenPerAcre','CostPerAcre','TrtType')
	dv.list[[i]] <- dv
}
...
###---> Calculate Ecological benefits and BCR by treatment and save output
setwd(paste0(wd,'/OUTPUT/Treatment_plan_ecobenefits/Supplementary')) #===> Change directory
for(i in 1:nrow(tspecs)){
	tbsave <- EBs[[i]]*ma_extent # Make version to save
	crs(tbsave) <- proj
	writeRaster(tbsave,paste0(tspecs$FB_Code[i],'_EB.tif'),format='GTiff',overwrite=T)
	bcr <- EBs[[i]]/Cs[[i]]
	bcr[Fs[[i]]==0] <- NA
	bcr <- bcr*ma_extent
	crs(bcr) <- proj
...
#-> Benefit-cost ratio
for(i in 1:nrow(tspecs)){
	tua$X <- tua[,paste0('T',i,'_EB')]/tua[,paste0('T',i,'_Cost')]
...
###---> Formulate model to maximize ecological benefits
...
###---> Treament Impact Analysis
#-> Estimate maximum budget for treatment impact curve
...
#-> Summary figure
tiff('Treatment_impact_curve.tif',width=1000,height=1100,compression='lzw',pointsize=22,
     type='windows')
par(mfrow=c(2,1))
...
#-> Treatment impact curve
ylim <- c(0,max(aia$ObjVal)*1.05)
plot(ObjVal~Budget,data=aia,type='l',xaxs='i',yaxs='i',ylim=ylim,lwd=2,axes=F,
     ylab='Ecological Benefit',xlab='Treatment budget ($)',main='Ecological Benefit by Budget')
...
plot(Total~Budget,data=aia,xaxs='i',yaxs='i',ylim=ylim,lwd=2,col=NA,axes=F,
     ylab='Treated Acres',xlab='Treatment budget ($)',main='Treatment Allocation')
for(i in 1:nrow(tspecs)){
