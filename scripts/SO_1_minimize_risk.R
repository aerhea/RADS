...
###########################################START MESSAGE############################################
log <- file(paste0(wd,'/OUTPUT/Treatment_plan_riskreduction/so_treatments.log'))
...
###---> Empty output folder
e <- try(setwd(paste0(wd,'/OUTPUT/Treatment_plan_riskreduction'))) #===> CD
...
###---> Risk reduction
# Note that you really don't need to do the pixel to acre conversion since the risk reduction 
...
for(i in 1:nrow(tspecs)){
	...
	rr <- rr*nfeas
	zt <- zonal(rr,rTUs,'mean',na.rm=T)
	tua$X <- zt[,'mean']*p2acres # Correct to per acre
...
# Correct to per acre
zt <- zonal(eNVC*p2acres,rTUs,'mean',na.rm=T)
tua$eNVC_mean <- zt[,'mean']
zt <- zonal(eNVC*p2acres,rTUs,'sum',na.rm=T)
tua$eNVC_Tot <- zt[,'sum']
