...
setwd('C:/Users/aerhe/Desktop/JCOS_RADS/scripts')
wd <- getwd()
...
gdal_proximity <- function(inRaster=NULL,inPath=NULL,outPath,returnRaster=T){
	path_2_gdal_function <- 'C:/Users/aerhe/AppData/Local/Programs/Python/Python37-32/Scripts/gdal_proximity.py'
	vp <- Sys.getenv('PROJ_LIB') # Get original variable path
	Sys.setenv(PROJ_LIB = 'C:/Program Files (x86)/GDAL/projlib') # Temporarilly reset
	if(!is.null(inRaster)){
...
BaseCost = 2500 # $/ac
slopeTH = 40 # percent
slopeMAX = 200 # percent  , max on full map was 260%
distTH = 800 # meters            
distMAX = 6400 # meters   , max on full map was 7273 m
...
# as infeasible. Similar criteria may be appropriate to include in other projects.
