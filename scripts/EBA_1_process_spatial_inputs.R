...
###########################################START MESSAGE############################################
#-> Start message
log <- file(paste0(wd,'/OUTPUT/EBA_extents/EBA1.log'))
...
e <- try(setwd(paste0(wd,'/OUTPUT/EBA_extents'))) #===> Change directory
...
if(class(e)=='try-error'){
	...
	d_list <- list.files()
	if(length(d_list) > 0){
		unlink(d_list[grep('EBA1.log',d_list,invert=T)],recursive=T)
	}
}
...
pdf('EBA_map_book.pdf',height=(ms$Height_pixels/200),width=(ms$Width_pixels/200))
...
for(i in 1:nrow(rs)){
	...
	#-> Save raster
	setwd(paste0(wd,'/OUTPUT/EBA_extents/GIS')) #===> Change directory
